...
def scrape_website():
    ...
    # 1. 配置区域 (Config)
    # ==============================
    target_url = 'https://en.wikipedia.org/wiki/List_of_presidents_of_the_United_States'
    ...
    for item in items:
        # A. 提取标题 (防御性编程)
        name_tag = item.find('span', class_='fn')
        if name_tag:
            name = name_tag.get_text(strip=True)
            date_tag = item.find('span', attrs={'style': 'font-size: 85%;'})
            if date_tag:
