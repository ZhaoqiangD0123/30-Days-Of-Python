# 第二天：30 Days for python programing

'''
练习： 1级
在 30DaysOfPython 文件夹内创建一个 day_2 文件夹。在这个文件夹里创建一个 variables.py 文件
输入注释 '第二天： 30 Days of python programming'
声明一个 first name 变量，并为它赋值
声明一个 last name 变量，并为它赋值
声明一个 full name 变量，并为它赋值
声明一个 country 变量，并为它赋值
声明一个 city 变量，并为它赋值
声明一个 age 变量，并为它赋值
声明一个 year 变量，并为它赋值
声明一个 is_married 变量，并为它赋值
声明一个 is_true 变量，并为它赋值
声明一个 is_light_on 变量，并为它赋值
在一行中声明多个变量
'''

first_name, last_name, full_name, country, city, age, year, is_married, is_true, is_light_on = 'Ding', 'Zhaoqiang', 'DingZhaoqiang', 'China', 'Xian', 21, 2025, False, True, True
'''
练习： 2级
使用 type() 内置函数检查你声明变量的数据类型
使用 len() 内置函数，算出你 first name 变量的长度
比较你 first name 和 last name 变量的长度
声明变量 num_one 为5，num_two 为4
将 num_one 和 num_two 相加，并赋值给 total 变量
将 num_one 和 num_two 相减，并赋值给 diff 变量
将 num_one 和 num_two 相乘，并赋值给 product 变量
将 num_one 和 num_two 相乘，并赋值给 division 变量
使用模数除法求出 num_two 除以 num_one 的结果，并将结果赋给变量 remainder
计算 num_one 的 num_two 次方并将值赋给变量 exp
计算 num_one 除以 num_two 商的整数部分（整除操作），并将结果赋给变量 floor_division
圆的半径为 30 米。
计算圆的面积并将值赋给名为 area_of_circle 的变量
计算圆的周长并将值赋给名为 circum_of_circle 的变量
将半径作为用户输入并计算面积。
使用内置输入函数从用户那里获取名字、姓氏、国家和年龄，并将值存储到相应的变量名中
在 Python shell 或文件中运行 help('keywords') 检查 Python 保留字或关键字
'''
print('first_name type is:', type(first_name))
print('last_name type is:', type(last_name))
print('full_name type is:', type(full_name))
print('country type is:', type(country))
print('city type is:', type(city))
print('age type is:', type(age))
print('year type is:', type(year))
print('is_married type is:', type(is_married))
print('is_true type is:', type(is_true))
print('is_light_on type is', type(is_light_on))

len_first_name = len(first_name)
print('the length of first_name is', len_first_name)
len_last_name = len(last_name)
if len_first_name > len_last_name:
    print('first_name is more longer')
elif len_first_name == len_last_name:
    print('first_name is equal to len_last_name')
else:
    print('last_name is more longer')

num_one, num_two = 5, 4
total, diff, product, division = num_one + num_two, num_one - num_two, num_one * num_two, num_one / num_two
remainder = num_two % num_one
exp = num_one ** num_two
floor_division = num_one // num_two
R_circle = 30  # m
area_of_circle = 3.14 * R_circle * R_circle
circum_of_circle = 2 * 3.14 * R_circle
R = int(input("请输入圆的半径(m):"))
if R >= 0:
    print('area_of_circle is', 3.14 * R * R)
else:
    print('Please input useful number')
last_name_in = str(input('Please input your last name:'))
first_name_in= str(input('Please input your first name:'))
country_in = str(input('Please input your Country:'))
age_in = int(input('Please input your age:'))
